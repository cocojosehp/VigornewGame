...
FSingleClimbPoint UHelpfulFunctionsBPLibrary::FindLedgePointStructure(const UObject* WorldContextObject, const ACharacter* TargetChar, FVector StartLocation, 
...
void UHelpfulFunctionsBPLibrary::TryFindLedgeLine(const UObject* WorldContextObject, bool& Valid, FSingleClimbPoint& LedgeStruct, const ACharacter* TargetChar, 
	...
	bool InverseTracing, int TraceDebugIndex, TArray<UClass*> NotThisClass, ECollisionChannel TraceChannel, float NormalFindOffset)
{
	...
	TArray<FSingleClimbPoint> PointsArrayStruct;
	FSingleClimbPoint BestPointStruct;
	FSingleClimbPoint PrePointData;
	FSingleClimbPoint LedgeFunctionReturn;
...
bool UHelpfulFunctionsBPLibrary::ClimbingLedgeValidP1(const UObject* WorldContextObject,bool IsValid, FSingleClimbPoint LeftStruct, FSingleClimbPoint RightStruct, ECollisionChannel TraceChannel, float UpAxisTollerance)
{
	if(IsValid==false)
	{ return false; }
...
FVector UHelpfulFunctionsBPLibrary::CalculateRelativeAcceleration(const UObject* WorldContextObject, const ACharacter* Character, FVector Acceleration, FVector Velocity)
{
	FVector ReturnVec;
	FQuat CharRotQuat = UKismetMathLibrary::Conv_RotatorToQuaternion(Character->GetActorRotation());
	if (UKismetMathLibrary::Dot_VectorVector(Acceleration, Velocity) > 0)
	{
		ReturnVec = UKismetMathLibrary::Vector_ClampSizeMax(Acceleration, Character->GetCharacterMovement()->GetMaxAcceleration());
		ReturnVec = ReturnVec / Character->GetCharacterMovement()->GetMaxAcceleration();
		ReturnVec = UKismetMathLibrary::Quat_UnrotateVector(CharRotQuat, ReturnVec);
	}
	else
	{
		ReturnVec = UKismetMathLibrary::Vector_ClampSizeMax(Acceleration, Character->GetCharacterMovement()->GetMaxBrakingDeceleration());
		ReturnVec = ReturnVec / Character->GetCharacterMovement()->GetMaxBrakingDeceleration();
		ReturnVec = UKismetMathLibrary::Quat_UnrotateVector(CharRotQuat, ReturnVec);
	}
	return ReturnVec;
}
...
void UHelpfulFunctionsBPLibrary::UpdateMovementValues(const UObject* WorldContextObject, const ACharacter* TargetChar, FVelocityBlendCpp VelocityBlend, float DeltaX, float VelocityBlendInterpSpeed, 
	...
	float& ReturnDiagonalScale, FVector& ReturnRelativeAcc, FVector2D& ReturnLeanAmount, float& ReturnWalkRunBlend, float& ReturnStrideBlend, float& ReturnStandingPlayRate, float& ReturnCrouchPlayRate)
{
	...
	const UAnimInstance* AnimBP = TargetChar->GetMesh()->GetAnimInstance();
	if (!IsValid(AnimBP))
	{ return; }
	// Calculate Smoothed Velocity Blend
	FVelocityBlendCpp TargetVelocityBlend = CalcVelocityBlendFast(AnimBP, Velocity);
	...
	ReturnRelativeAcc = CalculateRelativeAcceleration(TargetChar, TargetChar, Acceleration, Velocity);
...
void UHelpfulFunctionsBPLibrary::UpdateMovementValuesLod(const UObject* WorldContextObject, const ACharacter* TargetChar, FVector MovementLowDetail, FVector& ReturnMovementLowDetail, float& ReturnStandingPlayRate, 
	float& ReturnCrouchingPlayRate, float Speed, float DeltaX, const float AnimatedWalkSpeed, const float AnimatedRunSpeed, const float AnimatedSprintSpeed, const float AnimatedCrouchSpeed, FName WeightCurveName, bool ReturnOnlyDirection)
{
	...
	const float DirectionForwardHelper = UKismetMathLibrary::Dot_VectorVector(TargetChar->GetActorForwardVector(), UKismetMathLibrary::ClampVectorSize(TargetChar->GetVelocity(), -1, 1));
	const float DirectionRightHelper = UKismetMathLibrary::Dot_VectorVector(TargetChar->GetActorRightVector(), UKismetMathLibrary::ClampVectorSize(TargetChar->GetVelocity(), -1, 1));
...
void UHelpfulFunctionsBPLibrary::UpdateRotationValues(const UObject* WorldContextObject, const ACharacter* Character, int& ReturnDirection, float& FYaw, float& BYaw, float& LYaw, float& RYaw, 
	int Gait, int RotationMode, int MovementDirection, FRotator AimingRotation, FVector Velocity, UCurveVector* YawOffset_FB, UCurveVector* YawOffset_LR)
{
	...
	float DeltaControlRot = UKismetMathLibrary::NormalizedDeltaRotator(UKismetMathLibrary::MakeRotFromX(Velocity), Character->GetControlRotation()).Yaw;
	ReturnDirection = MD;
...
void UHelpfulFunctionsBPLibrary::FFT(TArray<float> Data, int32 NumSamples)
{
	...
	int32 PowerOfTwo = FMath::RoundToInt(FMath::Log2(NumSamples));
	...
	// Perform FFT
	int32 NumBits = FMath::RoundToInt(FMath::Log2(NumSamples));
